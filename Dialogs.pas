...
type
  ...
  TOpenOption = (ofReadOnly, ofOverwritePrompt, ofHideReadOnly, ofNoChangeDir, ofShowHelp, ofNoValidate, ofAllowMultiSelect,
    ofExtensionDifferent, ofPathMustExist, ofFileMustExist, ofCreatePrompt, ofShareAware, ofNoReadOnlyReturn,
    ofNoTestFileCreate, ofNoNetworkButton, ofNoLongNames, ofOldStyleDialog, ofNoDereferenceLinks, ofEnableIncludeNotify, ofEnableSizing,
  ...
  TFontDialogOption = (fdAnsiOnly, fdTrueTypeOnly, fdEffects, fdFixedPitchOnly, fdForceFontExist, fdNoFaceSel, fdNoOEMFonts, fdNoSimulations,
    fdNoSizeSel, fdNoStyleSel, fdNoVectorFonts, fdShowHelp, fdWysiwyg, fdLimitSize, fdScalableOnly, fdApplyButton);
  ...
  TPageSetupDialogOption = (psoDefaultMinMargins, psoDisableMargins, psoDisableOrientation, psoDisablePagePainting, psoDisablePaper,
    psoDisablePrinter, psoMargins, psoMinMargins, psoShowHelp, psoWarning, psoNoNetworkButton);
  TPageSetupDialogOptions = set of TPageSetupDialogOption;
  TPrinterKind = (pkDotMatrix, pkHPPCL);
  TPageType = (ptEnvelope, ptPaper);
  TPageSetupBeforePaintEvent = procedure(Sender: TObject; const PaperSize: SmallInt; const Orientation: TPrinterOrientation;
    const PageType: TPageType; var DoneDrawing: Boolean) of object;
  ...
  published
    ...
    property Options: TPageSetupDialogOptions read FOptions write FOptions default[psoDefaultMinMargins];
  ...
  TFindOption = (frDown, frFindNext, frHideMatchCase, frHideWholeWord, frHideUpDown, frMatchCase, frDisableMatchCase, frDisableUpDown,
  ...
  EPlatformVersionException = class(Exception);
    ...
    TFileDialogOption = (fdoOverWritePrompt, fdoStrictFileTypes, fdoNoChangeDir, fdoPickFolders, fdoForceFileSystem, fdoAllNonStorageItems,
      fdoNoValidate, fdoAllowMultiSelect, fdoPathMustExist, fdoFileMustExist, fdoCreatePrompt, fdoShareAware, fdoNoReadOnlyReturn,
      fdoNoTestFileCreate, fdoHideMRUPlaces, fdoHidePinnedPlaces, fdoNoDereferenceLinks, fdoDontAddToRecent, fdoForceShowHidden,
    ...
    TFileTypeItem = class(TCollectionItem)private FDisplayName: string;
  ...
  published
    ...
    TCustomFileSaveDialog = class(TCustomFileDialog)strict protected function CreateFileDialog: IFileDialog;
    override;
  end;
  ...
  type
    ...
    TTaskDialogFlag = (tfEnableHyperlinks, tfUseHiconMain, tfUseHiconFooter, tfAllowDialogCancellation, tfUseCommandLinks,
      tfUseCommandLinksNoIcon, tfExpandFooterArea, tfExpandedByDefault, tfVerificationFlagChecked, tfShowProgressBar, tfShowMarqueeProgressBar,
      tfCallbackTimer, tfPositionRelativeToWindow, tfRtlLayout, tfNoDefaultRadioButton, tfCanBeMinimized);
    ...
    const
      ...
      function MessageDlg(const Msg: string; DlgType: TMsgDlgType; Buttons: TMsgDlgButtons; HelpCtx: Longint): Integer; overload; inline;
      ...
      function MessageDlgPos(const Msg: string; DlgType: TMsgDlgType; Buttons: TMsgDlgButtons; HelpCtx: Longint; X, Y: Integer): Integer;
        overload; inline;
      function MessageDlgPos(const Msg: string; DlgType: TMsgDlgType; Buttons: TMsgDlgButtons; HelpCtx: Longint; X, Y: Integer;
        DefaultButton: TMsgDlgBtn): Integer; overload; inline;

      function MessageDlgPosHelp(const Msg: string; DlgType: TMsgDlgType; Buttons: TMsgDlgButtons; HelpCtx: Longint; X, Y: Integer;
      ...
      function TaskMessageDlg(const Title, Msg: string; DlgType: TMsgDlgType; Buttons: TMsgDlgButtons; HelpCtx: Longint): Integer; overload;
        inline;
      function TaskMessageDlg(const Title, Msg: string; DlgType: TMsgDlgType; Buttons: TMsgDlgButtons; HelpCtx: Longint;
        DefaultButton: TMsgDlgBtn): Integer; overload; inline;

      function TaskMessageDlgPos(const Title, Msg: string; DlgType: TMsgDlgType; Buttons: TMsgDlgButtons; HelpCtx: Longint;
        X, Y: Integer): Integer; overload; inline;
      function TaskMessageDlgPos(const Title, Msg: string; DlgType: TMsgDlgType; Buttons: TMsgDlgButtons; HelpCtx: Longint; X, Y: Integer;
        DefaultButton: TMsgDlgBtn): Integer; overload; inline;

      function TaskMessageDlgPosHelp(const Title, Msg: string; DlgType: TMsgDlgType; Buttons: TMsgDlgButtons; HelpCtx: Longint; X, Y: Integer;
        const HelpFileName: string): Integer; overload;
      function TaskMessageDlgPosHelp(const Title, Msg: string; DlgType: TMsgDlgType; Buttons: TMsgDlgButtons; HelpCtx: Longint; X, Y: Integer;
        const HelpFileName: string; DefaultButton: TMsgDlgBtn): Integer; overload;
...
begin
  ...
  SetWindowPos(Wnd, 0, Monitor.Left + ((Monitor.Width - Rect.Right + Rect.Left) div 2),
    Monitor.Top + ((Monitor.Height - Rect.Bottom + Rect.Top) div 3), 0, 0, SWP_NOACTIVATE or SWP_NOSIZE or SWP_NOZORDER);
end;
...
begin
  ...
  if Msg = WM_INITDIALOG then
    begin
      ...
      CreationControl.FDefWndProc := Pointer(SetWindowLong(Wnd, GWL_WNDPROC, Longint(CreationControl.FObjectInstance)));
...
type
  ...
  private
    ...
    procedure HandleShareViolation(Sender: TObject; var Response: TFileDialogShareViolationResponse);
  ...
  const
    CDialogOptionsMap: array [TOpenOption] of TFileDialogOptions = ([] { ofReadOnly } , [fdoOverWritePrompt], []
      { ofHideReadOnly } , [fdoNoChangeDir], []
      { ofShowHelp } , [fdoNoValidate], [fdoAllowMultiSelect], [fdoStrictFileTypes], [fdoPathMustExist], [fdoFileMustExist],
      [fdoCreatePrompt], [fdoShareAware], [fdoNoReadOnlyReturn], [fdoNoTestFileCreate], [] { ofNoNetworkButton } , [] { ofNoLongNames } , []
    ...
    if (Message.Msg = WM_INITDIALOG) and not(ofOldStyleDialog in Options) then
      ...
      begin
        ...
        case (LOFNotify.Hdr.code) of
          CDN_FILEOK: begin
              {$IF DEFINED(CLR)}
              LOpenFileName := TOpenFileName(Marshal.PtrToStructure(IntPtr(LOFNotify.lpOFN), TypeOf(TOpenFileName)));
              {$ELSE}
          ...
          CDN_INCLUDEITEM: if Message.LParam <> 0 then
              begin
                {$IF DEFINED(CLR)}
                LOFNotifyEx := TOFNotifyEx(Marshal.PtrToStructure(IntPtr(Message.LParam), TypeOf(TOFNotifyEx)));
                {$ELSE}
      ...
      if FileName = '' then
        ...
        begin
          if DirName[Length(DirName)] <> '\' then
            DirName := DirName + '\';
          repeat
            if (FileName[1] <> '\') and ((Length(FileName) <= 3) or (FileName[2] <> ':') or (FileName[3] <> '\')) then
...
const
  ...
  OpenOptions: array [TOpenOption] of DWORD = (OFN_READONLY, OFN_OVERWRITEPROMPT, OFN_HIDEREADONLY, OFN_NOCHANGEDIR, OFN_SHOWHELP,
    OFN_NOVALIDATE, OFN_ALLOWMULTISELECT, OFN_EXTENSIONDIFFERENT, OFN_PATHMUSTEXIST, OFN_FILEMUSTEXIST, OFN_CREATEPROMPT,
    OFN_SHAREAWARE, OFN_NOREADONLYRETURN, OFN_NOTESTFILECREATE, OFN_NONETWORKBUTTON, OFN_NOLONGNAMES, OFN_EXPLORER, OFN_NODEREFERENCELINKS,
...
begin
  if (Win32MajorVersion >= 6) and UseLatestCommonDialogs and (Template = '') and not
    (Assigned(FOnIncludeItem) or Assigned(FOnClose) or Assigned(FOnShow)) then
    begin
  ...
  with OpenFileName do
    begin
      ...
      ((Win32Platform = VER_PLATFORM_WIN32_WINDOWS) and (Win32MajorVersion >= 4) and (Win32MinorVersion >= 90)) then { WinME }
        ...
        lStructSize := Marshal.Sizeof(TypeOf(TOpenFileName)) - (Sizeof(DWORD) shl 1) - Sizeof(IntPtr); { subtract size of added fields }
  ...
  try
    ...
    try
      ...
      if Result then
        begin
          OpenFileName := TOpenFileName(Marshal.PtrToStructure(Mem, TypeOf(TOpenFileName)));
          GetFileNames(OpenFileName);
    ...
    // just in case....
    Marshal.WriteInt32(Mem, Integer(Marshal.OffsetOf(TypeOf(TOpenFileName), 'lpstrFile')), 0);
...
begin
  if (Win32MajorVersion >= 6) and UseLatestCommonDialogs and (Template = nil) and not
    (Assigned(FOnIncludeItem) or Assigned(FOnClose) or Assigned(FOnShow)) then
    begin
      // This requires Windows Vista or later
...
const
  DialogOptions: array [TColorDialogOption] of DWORD = (CC_FULLOPEN, CC_PREVENTFULLOPEN, CC_SHOWHELP, CC_SOLIDCOLOR, CC_ANYCOLOR);
  ColorPrefix = 'Color';
var
  ...
  begin
    for I := 0 to MaxCustomColors - 1 do
      begin
        ...
        {$IFEND}
        FCustomColors.Values[ColorPrefix + Char(Ord('A') + I)] := Format('%.6x', [Val]);
      end;
  end;
{$IF DEFINED(CLR)}
  ...
  begin
    ...
    for I := 0 to MaxCustomColors - 1 do
      begin
        ...
        if Value <> '' then
          Marshal.WriteInt32(CustomColorsArray, I * Sizeof(Longint), StrToInt('$' + Value))
        else
...
begin
  with ChooseColorRec do
    begin
        ...
        if Length(Template) > 0 then
          begin
            ...
            try
              ...
              if Result then
                begin
                  ChooseColorRec := TChooseColor(Marshal.PtrToStructure(DialogData, TypeOf(TChooseColor)));
      ...
      begin
        if (Msg = WM_COMMAND) and ((WParam and $FFFF) = IDAPPLYBTN) and (((WParam shr 16) and $FFFF) = BN_CLICKED) then
          begin
            DoApply(Wnd);
            Result := 1;
          end
      ...
      begin
        if (Msg = WM_COMMAND) and (LongRec(WParam).Lo = IDAPPLYBTN) and (LongRec(WParam).Hi = BN_CLICKED) then
          begin
      ...
      begin
        ...
        if (Message.Msg = WM_COMMAND) and (Message.WParamHi = CBN_SELENDOK) then
          if (Message.WParamLo = cmb4) then
            FFontColorModified := True
          else if (Message.WParamLo = cmb5) then
      ...
      const
        FontOptions: array [TFontDialogOption] of DWORD = (CF_ANSIONLY, CF_TTONLY, CF_EFFECTS, CF_FIXEDPITCHONLY, CF_FORCEFONTEXIST,
          CF_NOFACESEL, CF_NOOEMFONTS, CF_NOSIMULATIONS, CF_NOSIZESEL, CF_NOSTYLESEL, CF_NOVECTORFONTS, CF_SHOWHELP,
          CF_WYSIWYG or CF_BOTH or CF_SCALABLEONLY, CF_LIMITSIZE, CF_SCALABLEONLY, CF_APPLY);
        Devices: array [TFontDialogDevice] of DWORD = (CF_SCREENFONTS, CF_PRINTERFONTS, CF_BOTH);
      var
      ...
      begin
              ...
              begin
                ...
                try
                  ...
                  try
                    ...
                    if Result then
                      begin
                        ChooseFontRec := TChooseFont(Marshal.PtrToStructure(DialogData, TypeOf(TChooseFont)));
                    ...
                    Marshal.WriteInt32(DialogData, Integer(Marshal.OffsetOf(TypeOf(TChooseFont), 'lpLogFont')), 0);
        ...
        begin
          ...
          if DevMode <> nil then
            begin
              ...
              DeviceMode := GlobalAlloc(GHND, Sizeof(TDeviceMode) + (CCHDEVICENAME + CCHFORMNAME) * Marshal.SystemDefaultCharSize);
        ...
        begin
          ...
          if DeviceMode <> 0 then
            begin
              DeviceNames := GlobalAlloc(GHND, Sizeof(TDevNames) + (StrLen(Device) + StrLen(Driver) + StrLen(Port) + 3) * Sizeof(Char));
        ...
        begin
          ...
          try
            with DevNames^ do
              Printer.SetPrinter(PChar(DevNames) + wDeviceOffset, PChar(DevNames) + wDriverOffset, PChar(DevNames) + wOutputOffset, DeviceMode);
          finally
            GlobalUnlock(DeviceNames);
            GlobalFree(DeviceNames);
          end;
        ...
        {$ELSE}
          ...
          begin
            {$IF NOT DEFINED(CLR)}
            Result := UINT(False);
            if not Assigned(PgSetupDlg) then
              Exit;
            with PgSetupDlg do
              {$IFEND}
              begin
                  ...
                  begin
                    if Assigned(FBeforePaint) then
                      ...
                      BeforePaint(PgSetupDlg, (WParam and $FFFF),
                        {$IFEND}
                        Orientation[(WParam shr 16) in [$0001, $0003, $000B, $0019]], PageType[(WParam shr 16) > $000B], DoneDrawing);
                  end
                else
                  case Message of
          ...
          begin
            ...
            with FPageSetupDlgRec do
              begin
                ...
                if psoMinMargins in FOptions then
            ...
            try
              ...
              Result := TaskModalDialog(DialogData);
              FPageSetupDlgRec := TPageSetupDlg(Marshal.PtrToStructure(DialogData, TypeOf(TPageSetupDlg)));
            finally
        ...
        {$IF DEFINED(CLR)}
          ...
          var
            ...
            function FindReplaceWndProc(Wnd: HWnd; Msg, WParam, LParam: Longint): Longint; stdcall;
              ...
              begin
                Result := CallWindowProc(Pointer(GetProp(Wnd, MakeIntAtom(WndProcPtrAtom))), Wnd, Msg, WParam, LParam);
              end;
            {$IFEND}

            begin
              case Msg of
                WM_DESTROY: if Application.DialogHandle = Wnd then
                    Application.DialogHandle := 0;
                WM_NCACTIVATE: if WParam <> 0 then
                    begin
          ...
          {$IF DEFINED(CLR)}
                  ...
                  begin
                    ...
                    with TFindDialog(PFindReplace(LParam)^.lCustData) do
                      ...
                      if (Left <> -1) or (Top <> -1) then
                        SetWindowPos(Wnd, 0, Left, Top, 0, 0, SWP_NOACTIVATE or SWP_NOSIZE or SWP_NOZORDER);
                    {$IF DEFINED(CLR)}
            ...
            const
              FindOptions: array [TFindOption] of DWORD = (FR_DOWN, FR_FINDNEXT, FR_HIDEMATCHCASE, FR_HIDEWHOLEWORD, FR_HIDEUPDOWN,
                FR_MATCHCASE, FR_NOMATCHCASE, FR_NOUPDOWN, FR_NOWHOLEWORD, FR_REPLACE, FR_REPLACEALL, FR_WHOLEWORD, FR_SHOWHELP);

              { TFindDialog }
              {$IF DEFINED(CLR)}
            const
              FindReplaceStringLen = 256;
            ...
            begin
              if FFindHandle <> 0 then
                SendMessage(FFindHandle, WM_CLOSE, 0, 0);
              inherited Destroy;
            end;
          {$IFEND}
            ...
            begin
              ...
              if not Result then
                ...
                if (Cardinal(Msg.Msg) = FFindMsg) then
                  begin
                    FFindReplace := TFindReplace(Marshal.PtrToStructure(IntPtr(Msg.LParam), TypeOf(TFindReplace)));
                  ...
                  begin
                    ...
                    if Assigned(FResources) and (FResources.FFindHandle <> 0) then
                      with FResources do
                        ...
                        if FFindHandle <> 0 then
                          {$IFEND}
                          SetWindowPos(FFindHandle, 0, Value.X, Value.Y, 0, 0, SWP_NOACTIVATE or SWP_NOSIZE or SWP_NOZORDER);
                  end;
              end;
              ...
              type
                ...
                public
                  ...
                  function OnFolderChanging(const pfd: IFileDialog; const psiFolder: IShellItem): HResult; {$IFNDEF CLR} stdcall; {$ENDIF}
                  ...
                  function OnShareViolation(const pfd: IFileDialog; const psi: IShellItem; out pResponse: DWORD): HResult;
                  {$IFNDEF CLR} stdcall; {$ENDIF}
                  function OnTypeChange(const pfd: IFileDialog): HResult; {$IFNDEF CLR} stdcall; {$ENDIF}
                  function OnOverwrite(const pfd: IFileDialog; const psi: IShellItem; out pResponse: DWORD): HResult; {$IFNDEF CLR} stdcall;
                  {$ENDIF}
                end;
              ...
              function TFileDialogEvents.OnFolderChanging(const pfd: IFileDialog; const psiFolder: IShellItem): HResult;
              begin
              ...
              function TFileDialogEvents.OnOverwrite(const pfd: IFileDialog; const psi: IShellItem; out pResponse: DWORD): HResult;
              begin
              ...
              function TFileDialogEvents.OnShareViolation(const pfd: IFileDialog; const psi: IShellItem; out pResponse: DWORD): HResult;
              begin
              ...
              const
                CDialogOptions: array [TFileDialogOption] of DWORD = (FOS_OVERWRITEPROMPT, FOS_STRICTFILETYPES, FOS_NOCHANGEDIR,
                  FOS_PICKFOLDERS, FOS_FORCEFILESYSTEM, FOS_ALLNONSTORAGEITEMS, FOS_NOVALIDATE, FOS_ALLOWMULTISELECT, FOS_PATHMUSTEXIST,
                  FOS_FILEMUSTEXIST, FOS_CREATEPROMPT, FOS_SHAREAWARE, FOS_NOREADONLYRETURN, FOS_NOTESTFILECREATE, FOS_HIDEMRUPLACES,
                  FOS_HIDEPINNEDPLACES, FOS_NODEREFERENCELINKS, FOS_DONTADDTORECENT, FOS_FORCESHOWHIDDEN, FOS_DEFAULTNOMINIMODE,
                    ...
                    with FDialog do
                      begin
                        ...
                        if FDefaultFolder <> '' then
                          begin
                            if Succeeded(SHCreateItemFromParsingName({$IFNDEF CLR} PWideChar
                                {$ENDIF}(WideString(FDefaultFolder)), nil, StringToGUID(SID_IShellItem), LShellItem)) then
                        ...
                        for LPlace in FFavoriteLinks do
                          if Succeeded(SHCreateItemFromParsingName({$IFNDEF CLR} PWideChar
                              {$ENDIF}(WideString(LPlace.Location)), nil, StringToGUID(SID_IShellItem), LShellItem)) then
              ...
              function TCustomFileDialog.ShareViolation(psiFile: IShellItem; var Response: Cardinal): HResult;
              var
              ...
              begin
                SendMessage(FClient.Handle, TDM_SET_MARQUEE_PROGRESS_BAR, Windows.WParam(Bool((tfShowMarqueeProgressBar in FClient.Flags))), 0);
              ...
              begin
                if Client.Handle <> 0 then
                  SendMessage(Client.Handle, TDM_SET_BUTTON_ELEVATION_REQUIRED_STATE, ModalResult, LParam(FElevationRequired))
              end;

              function TTaskDialogButtonItem.GetButtonText: string;
              begin
                if (FCommandLinkHint <> '') and ((tfUseCommandLinks in Client.Flags) or (tfUseCommandLinksNoIcon in Client.Flags)) then
              ...
              function TCustomTaskDialog.CallbackProc(HWnd: HWnd; Msg: UINT; WParam: WParam; LParam: LParam; lpRefData: LONG_PTR): HResult;
              var
                LCanClose, LReset: Boolean;
              begin
                Result := S_OK;
                case Msg of
                  TDN_BUTTON_CLICKED: begin
                      LCanClose := True;
                      DoOnButtonClicked(WParam, LCanClose);
                      if not LCanClose then
                        Result := S_FALSE;
                    end;
                  TDN_CREATED: DoOnDialogCreated;
                  TDN_DESTROYED: DoOnDialogDestroyed;
                  ...
                  TDN_EXPANDO_BUTTON_CLICKED: DoOnExpandButtonClicked(Bool(WParam));
                  TDN_HELP: DoOnHelp;
                  TDN_HYPERLINK_CLICKED: DoOnHyperlinkClicked(WStrPtrToString(LPCWSTR(LParam)));
                  TDN_NAVIGATED: DoOnNavigated;
                  TDN_RADIO_BUTTON_CLICKED: DoOnRadioButtonClicked(WParam);
                  TDN_TIMER: begin
                      LReset := False;
                  ...
                  TDN_VERIFICATION_CLICKED: DoOnVerificationClicked(WParam = 1);
                end;
              end;
              {$IF NOT DEFINED(CLR)}
              ...
              begin
                Result := TCustomTaskDialog(Pointer(lpRefData)).CallbackProc(HWnd, Msg, WParam, LParam, 0);
              end;
              {$IFEND}
              ...
              const
                CTaskDlgFlags: array [TTaskDialogFlag] of Cardinal = (TDF_Enable_Hyperlinks, TDF_Use_Hicon_Main, tdf_Use_Hicon_Footer,
                  TDF_ALLOW_DIALOG_CANCELLATION, TDF_USE_COMMAND_LINKS, TDF_USE_COMMAND_LINKS_NO_ICON, TDF_EXPAND_FOOTER_AREA,
                  TDF_EXPANDED_BY_DEFAULT, TDF_VERIFICATION_FLAG_CHECKED, TDF_SHOW_PROGRESS_BAR, TDF_SHOW_MARQUEE_PROGRESS_BAR,
                  TDF_CALLBACK_TIMER, TDF_POSITION_RELATIVE_TO_WINDOW, TDF_RTL_LAYOUT, TDF_NO_DEFAULT_RADIO_BUTTON, TDF_CAN_BE_MINIMIZED);

                CTaskDlgCommonButtons: array [TTaskDialogCommonButton] of Cardinal = (TDCBF_OK_BUTTON, TDCBF_YES_BUTTON, TDCBF_NO_BUTTON,
                  TDCBF_CANCEL_BUTTON, TDCBF_RETRY_BUTTON, TDCBF_CLOSE_BUTTON);

                CTaskDlgDefaultButtons: array [TTaskDialogCommonButton] of Integer = (IDOK, IDYES, IDNO, IDCANCEL, IDRETRY, IDCLOSE);
              ...
              begin
                if Value <> FFlags then
                  begin
                    if (tfVerificationFlagChecked in FFlags) <> (tfVerificationFlagChecked in Value) and (FHandle <> 0) then
                      SendMessage(FHandle, TDM_CLICK_VERIFICATION, WParam((tfVerificationFlagChecked in Value)), LParam(False));
                    FFlags := Value;
                  end;
              end;
              ...
              function TaskMessageDlg(const Title, Msg: string; DlgType: TMsgDlgType; Buttons: TMsgDlgButtons; HelpCtx: Longint;
                DefaultButton: TMsgDlgBtn): Integer; overload;
              begin
                Result := TaskMessageDlgPosHelp(Title, Msg, DlgType, Buttons, HelpCtx, -1, -1, '', DefaultButton);
              end;
              ...
              begin
                Result := TaskMessageDlgPosHelp(Title, Msg, DlgType, Buttons, HelpCtx, X, Y, '', DefaultButton);
              end;
              ...
              function DoTaskMessageDlgPosHelp(const Instruction, Msg: string; DlgType: TMsgDlgType; Buttons: TMsgDlgButtons; HelpCtx: Longint;
                X, Y: Integer; const HelpFileName: string): Integer; overload; forward;

              function DoTaskMessageDlgPosHelp(const Instruction, Msg: string; DlgType: TMsgDlgType; Buttons: TMsgDlgButtons; HelpCtx: Longint;
                X, Y: Integer; const HelpFileName: string; DefaultButton: TMsgDlgBtn): Integer; overload; forward;
              ...
              begin
                if (Win32MajorVersion >= 6) and UseLatestCommonDialogs and ThemeServices.ThemesEnabled then
                  Result := DoTaskMessageDlgPosHelp('', Msg, DlgType, Buttons, HelpCtx, X, Y, HelpFileName, DefaultButton)
                else
                  Result := DoMessageDlgPosHelp(CreateMessageDialog(Msg, DlgType, Buttons, DefaultButton), HelpCtx, X, Y, HelpFileName);
              end;

              [UIPermission(SecurityAction.LinkDemand, Window = UIPermissionWindow.SafeSubWindows)]

              function TaskMessageDlgPosHelp(const Title, Msg: string; DlgType: TMsgDlgType; Buttons: TMsgDlgButtons; HelpCtx: Longint;
                X, Y: Integer; const HelpFileName: string): Integer;
              begin
                if (Win32MajorVersion >= 6) and ThemeServices.ThemesEnabled then
                  Result := DoTaskMessageDlgPosHelp(Title, Msg, DlgType, Buttons, HelpCtx, X, Y, HelpFileName)
                else
                  Result := DoMessageDlgPosHelp(CreateMessageDialog(Msg, DlgType, Buttons), HelpCtx, X, Y, HelpFileName);
              end;
              ...
              function TaskMessageDlgPosHelp(const Title, Msg: string; DlgType: TMsgDlgType; Buttons: TMsgDlgButtons; HelpCtx: Longint;
                X, Y: Integer; const HelpFileName: string; DefaultButton: TMsgDlgBtn): Integer;
              begin
                if (Win32MajorVersion >= 6) and ThemeServices.ThemesEnabled then
                  Result := DoTaskMessageDlgPosHelp(Title, Msg, DlgType, Buttons, HelpCtx, X, Y, HelpFileName, DefaultButton)
                else
                  Result := DoMessageDlgPosHelp(CreateMessageDialog(Msg, DlgType, Buttons, DefaultButton), HelpCtx, X, Y, HelpFileName);
              end;
              ...
              const
                ...
                LModalResults: array [TMsgDlgBtn] of Integer = (mrYes, mrNo, mrOk, mrCancel, mrAbort, mrRetry, mrIgnore, mrAll, mrNoToAll,
                  mrYesToAll, tdbHelp, mrClose);
              var
                DlgBtn: TMsgDlgBtn;
              ...
              begin
                if mbOK in Buttons then
                  ...
                  DefaultButton := mbRetry;
                Result := DoTaskMessageDlgPosHelp(Instruction, Msg, DlgType, Buttons, HelpCtx, X, Y, HelpFileName, DefaultButton);
              end;
              ...
              begin
                ...
                WndProcPtrAtom := GlobalAddAtom(StrFmt(AtomText, 'WndProcPtr%.8X%.8X', [hInstance, GetCurrentThreadID]));
              end;
